...
# Defines
AODV_HELLO_INTERVAL         =   100
AODV_HELLO_TIMEOUT          =   300
AODV_PATH_DISCOVERY_TIME    =   300
...
class aodv(threading.Thread):
    ...
    # Processs incoming generate key
    def aodv_process_send_qubits(self,message):
        # Get the message contents, sender and receiver
        sender = message[1]
        receiver = message[2]
        msg = message[3]
        ...
        # Check if the message is for us
        if (receiver == self.node_id):
            # Add the qubits to the qubits box
            self.qubits[msg] = {'Sender' : sender, 'Message':msg}
            #log the message and notify the user
            logging. debug(message)
            print("Qubits are arrived. Issue the 'show_qubits' to see the qubits")
            ...
            message = message[0] + ":" + message[1] + ":" + message[2] + ":" + message[3]
            self.aodv_send(next_hop, next_hop_port, message)
            print("Qubits are forwarded to: ",next_hop,"............")
            #print("Qubits are ",msg)
            logging.debug("['SEND_QUBITS', '" + sender + " to " + receiver + "', " + msg + "']")
    
    # Process incoming bases
    def aodv_process_send_bases(self,message):
        # Get the message contents, sender and receiver
        sender = message[1]
        receiver = message[2]
        msg = message[3]
        ...
        if (receiver == self.node_id):
            # Add the qubits to the qubits box
            self.bases[msg] = {'Sender' : sender, 'Message':msg}
            #log the message and notify the user
            logging. debug(message)
            print("Bases are arrived. Issue the 'show_bases' to see the bases")
            ...
            message = message[0] + ":" + message[1] + ":" + message[2] + ":" + message[3]
            self.aodv_send(next_hop, next_hop_port, message)
            print("Bases are forwarded to: ",next_hop,"............")
            #print("Bases are ",msg)
            logging.debug("['SEND_BASES', '" + sender + " to " + receiver + "', " + msg + "']")
    ...
    # Process an incoming RREP encoded message
    ...
    # creating the vector of length 100 bits/bases
    def vector(self,length = 100):
        return list(randint(0,2,length))


    # Encoding the bits with bases of the Sender
    def encode_message(self,sender_bits,sender_bases):
        bits = sender_bits
        bases = sender_bases
        qubits = ""
        for i in range(100):
            ...
            if bases[i] == 0: # Prepare qubit in Z-basis
                if bits[i] == 0:
                    ...
                    qubit = '+' #change into the hadamard gate 
    ...
    # Measure the qubits in the receiver bases
    def measure(self,message,receiver_bases):
        results = []
        for i in range(100):
    ...
    # Measuring the key
    def key(self,sender_bases,reciever_bases,bits):
        #print(type(sender_bases),type(reciever_bases),type(bits))
        sender_bases = "".join(str(i) for i in sender_bases)
        reciever_bases = "".join(str(i) for i in reciever_bases)
        bits = "".join(str(i) for i in bits)
        quantum_key = []
        for q in range(100):
            if sender_bases[q] == reciever_bases[q]:
                quantum_key.append(bits[q])
        l = len(quantum_key)
        if l>=32 and l<=448:
            if l%8 == 0:
                pass
                ...
                while(l%8!=0):
                    quantum_key.append('0')
                    l = len(quantum_key)
        return quantum_key

    # Generate the Quantum key between the sender and reciever
    def aodv_send_qubits(self,from_tester):
        ...
        if (from_tester == False):
            ...
            dest = command[2]
            #message_data = command[3]
        self.sender_bits = self.vector()
        self.sender_bases = self.vector()
        self.sender_qubits=self.encode_message(self.sender_bits,self.sender_bases)
        #bases = "".join(str(i) for i in self.sender_bases)
        message_data = self.sender_qubits
        message_type = "SEND_QUBITS"
        ...
        if dest in self.routing_table.keys():
            ...
            if (destination['Status'] == 'Inactive'):
                ...
                print("Qubits are forwarded to next_hop",next_hop,"............")
                #print("Qubits  ",message_data)
                ...
                logging.debug("['SEND_QUBITS', '" + source + " to " + dest + " via " + next_hop + "', '" + message_data + "']")
                print("Qubits sent")
        else:
    ...
    def aodv_send_bases(self,from_tester):
        # Get the command sent by the listener thread / tester process
        command = self.command
        if (from_tester == False):
            source = command[1]
            ...
            #message_data = command[3]
        #self.sender_bits = self.vector()
        if self.sender_bases == []:
            self.sender_bases = self.vector()
        #self.sender_qubits=self.encode_message(self.sender_bits,self.sender_bases)
        bases = "".join(str(i) for i in self.sender_bases)
        message_data = bases
        message_type = "SEND_BASES"
        message = message_type + ":" + source + ":" + dest + ":" + message_data
        ...
        if dest in self.routing_table.keys():
            ...
            if (destination['Status'] == 'Inactive'):
                ...
                next_hop = destination['Next-Hop']
                next_hop_port = destination['Next-Hop-Port']
                print("Bases are forwarded to next_hop",next_hop,"............")
                #print("Bases  ",message_data)
                self.aodv_send(next_hop, int(next_hop_port), message)
                # TODO: update lifetime here as the route was used
                self.aodv_restart_route_timer(destination, False)
                logging.debug("['SEND_BASES', '" + source + " to " + dest + " via " + next_hop + "', '" + message_data + "']")
                print("Bases sent")
        else:
            # Initiate a route discovery message to the destination
            self.aodv_send_rreq(dest, -1)
    ...
    def aodv_send_message(self, from_tester):
        ...
        hash_code = QHFM.QHFM296(message_data)
        ...
        if (user=='y'):
            quantum_key = self.key(self.sender_bases, self.receiver_bases, self.sender_bits)
            quantum_key = "".join(i for i in quantum_key)
            print("The quantum key is ",quantum_key," and the length of the key is ",len(quantum_key))
            key = int(quantum_key,2)
            message_data=algo.encrypt_msg(message_data,str(key))
            print("Hash Code: ",hash_code)
            print("Encrypted message: ",message_data)
            message_data+=' '+hash_code
        message = message_type + ":" + source + ":" + dest + ":" + message_data
    ...
    def aodv_show_routing_table(self, from_tester):
        ...
        print("There are "+str(len(self.routing_table))+" active links in the routing table")
        print("")
        ...
        print("")'''
        print("Node     Next-link")
        print("----------------------------")
        for r in self.routing_table.values():
            print(str(self.node_id)+"  <------->  "+str(r['Next-Hop']))
        print("")
        self.status = "Success"
    ...
    # Return the show bases back to the node
    def aodv_show_bases(self,from_tester):
        print("")
        print("There are " + str(len(self.bases)) + " bases in the base-box")
        print("")
        
        print("Sender     Bases")
        print("------------------")
        sender = ""
        for m in self.bases.values():
            #self.qubits = m['Message']
            sender = m['Sender']
            print(m['Sender'] + "         " + m['Message'])
            print("The bases are : ",m['Message'])
            self.receiver_bases = m['Message']
        print("")
        print("Bases are recieved from sender send your bases to ",sender)
        self.status = "Success"
    
    # Return the show qubits  back to the node
    def aodv_show_qubits(self,from_tester):
        print("")
        print("There are " + str(len(self.qubits)) + " qubits in the qubit_box")
        print("")
        
        print("Sender     Qubits")
        print("------------------")
        sender = ""
        for m in self.qubits.values():
            sender = m['Sender']
            print(m['Sender'] + "         " + m['Message'])
            self.sender_qubits = m['Message']
            print("The Qubits are : ",m['Message'])
        print("")
        print("Qubits are recieved from sender. Send your bases to sender ",sender)
        self.status = "Success"
    ...
    def aodv_show_messages(self, from_tester):
        ...
        print("------------------")
        self.receiver_results = self.measure(self.sender_qubits,self.sender_bases)
        for m in self.message_box.values():
            print(m['Sender'] + "         " + m['Message'])
            user=input("Do you want to decrypt [y/n] ? ")
            if user=='y':
                quantum_key = self.key(self.receiver_bases,self.sender_bases,self.receiver_results)
                quantum_key = "".join(str(i) for i in quantum_key)
                print("The quantum key is ",quantum_key," and the length of the key is ",len(quantum_key))
                key = int(quantum_key,2)
                msg,shash_code=m["Message"].split(' ')
                d_msg = algo.decrypt_msg(msg,str(key))
                rhash_code = QHFM.QHFM296(d_msg)
                if rhash_code == shash_code:
                    print("......The Message digest is verified........")
                    print("Hash Code: ",rhash_code)
                    print("Decrypted message: ",d_msg)
                else:
                    print("Message digest is not verified")
            ...
            for r in readable:
                if r is self.listener_sock:
                    ...
                    if command_type == "NODE_ACTIVATE":
                        ...
                        self.aodv_delete_messages(False)
                    elif command_type == "SEND_QUBITS":
                        self.aodv_send_qubits(False)
                    elif command_type == "SEND_BASES":
                        self.aodv_send_bases(False)
                    ...
                    elif command_type == "SHOW_MESSAGES":
                        self.aodv_show_messages(False)
                    elif command_type == "SHOW_QUBITS":
                        self.aodv_show_qubits(False)
                    elif command_type == "SHOW_BASES":
                        self.aodv_show_bases(False)
                    else:
                        self.aodv_default()
                    ...
                    elif (message_type == "SEND_QUBITS"):
                        self.aodv_process_send_qubits(message)
                    elif (message_type == "SEND_BASES"):
                        self.aodv_process_send_bases(message)
