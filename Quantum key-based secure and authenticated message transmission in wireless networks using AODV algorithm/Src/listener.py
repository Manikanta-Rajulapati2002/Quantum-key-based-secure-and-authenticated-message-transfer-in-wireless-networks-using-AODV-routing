...
class listener(threading.Thread):
    ...
    def help(self):
        ...
        print("send_qubits     : Generate quantum key between sender and receiver")
        print("send_bases      : Send bases to sender/receiver")
        ...
        print("show_qubits     : View the encoded messages received so far")
    ...
    # Generate the Quantum key betweeen the sender and reciever
    def send_qubits(self):
        node = input("Enter the destination: ")
        message_type = "SEND_QUBITS"
        message = message_type + ":" + self.node_id + ":" + node
        message_bytes = bytes(message,'utf-8')
        self.send(message_bytes)
        self.wait_for_response()

    # Send bases to the receiver    
    def send_bases(self):
        print("Send bases to ")
        node = input("Enter the destination: ")
    ...
    def show_messages(self):
        message_type = "SHOW_MESSAGES"
        message = message_type + ":" + ""
        message_bytes = bytes(message, 'utf-8')
        self.send(message_bytes)
        self.wait_for_response()

    # Display the qubits sent to this node by other nodes
    def show_qubits(self):
        message_type = 'SHOW_QUBITS'
        message = message_type + ":" + ""
        message_bytes = bytes(message,'utf-8')
        self.send(message_bytes)
        self.wait_for_response()

    # Display the bases sent to this node by other nodes
    def show_bases(self):
        message_type = 'SHOW_BASES'
        message = message_type + ":" + ""
        message_bytes = bytes(message,'utf-8')
        self.send(message_bytes)
        self.wait_for_response()
    ...
    def run(self):
        ...
        while (True):
            ...
            {'help'             : self.help,
             ...
             'send_qubits'      : self.send_qubits,
             'show_qubits'      : self.show_qubits,
             'send_bases'       : self.send_bases,
             'show_bases'       : self.show_bases}.get(command, self.default)()
